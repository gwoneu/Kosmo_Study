...
router.get("/cart.json", function(req, res){
    ...
    let sql=`select * from view_cart where cid in (${cart})`; //주문도서목록
    db.get().query(sql, function(err, rows){
        const order=rows;
        sql='select * from users where uid=?'; //사용자정보
...
//주문목록.json
router.get('/list.json', function(req, res){ //localhost:3000/order/list.json?uid=red
    const uid=req.query.uid;
...
//주문상품목록.json
router.get('/book.json', function(req, res){ //localhost:3000/order/book.json?pid=11
