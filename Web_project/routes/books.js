...
router.post('/like/insert', function(req, res){
    const uid=req.body.uid; //post면 body에 저장되어 있다.
    const bid=req.body.bid; //get은 query
...
router.get('/like/check', function(req, res){
    const uid=req.query.uid;
    const bid=req.query.bid;
    const sql='select count(*) fcnt,(select count(*) from favorite where bid=? and uid=?) ucnt from favorite where bid=?';
